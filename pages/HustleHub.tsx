...
const initialHustles: HustleIdea[] = [
  { id: '1', title: 'Niche Newsletter', description: 'Curate high-value content for a specific industry (e.g., AI for Lawyers). Monetize via subscriptions.', difficulty: 'Medium', potential: '$2k - $10k/mo', tags: ['Content', 'Recurring'], riskLevel: 'Low', timeToRevenue: '3-6 Months', content: `
...
- **Premium Tier:** Launch a paid tier with exclusive deep-dives, community access, or early access. Promote it to your free subscribers.` },
  { id: '2', title: 'SaaS Micro-Tools', description: 'Build small, single-purpose tools (e.g., PDF compressor) and charge a small monthly fee.', difficulty: 'Hard', potential: '$5k+/mo', tags: ['Code', 'Scale'], riskLevel: 'Medium', timeToRevenue: '1-3 Months', content: `
...
- **Collect Feedback:** Listen to your first users. Iterate on the product based on their feedback to achieve product-market fit. Charge from day one.` },
  { id: '3', title: 'High-Ticket Sales', description: 'Close calls for influencers or agencies. Commission based only. High skill ceiling.', difficulty: 'Medium', potential: '$10k+/mo', tags: ['Sales', 'Service'], riskLevel: 'Low', timeToRevenue: '1 Month', content: `
### Step 1: Master the Skill
...
const initialGuides: HustleGuide[] = [
  { 
    ...
    category: 'Starter',
    content: `
### Step 1: Define Your High-Value Skill
...
const initialFinancialRules: FinancialRule[] = [
    { id: 'f1', title: 'The 50/30/20 Rule', description: 'Allocate 50% of income to Needs, 30% to Wants, and 20% to Savings & Debt Repayment. A simple foundation for financial discipline.', category: 'Saving', content: `
**The 50/30/20 rule is a foundational budgeting framework.** It provides clarity and removes decision fatigue from your financial life.
...
- **20% for Financial Goals:** This is non-negotiable. This portion goes directly to your future self. It includes paying off high-interest debt (like credit cards), building your emergency fund, and investing for retirement.`
    ...
    { id: 'f2', title: 'Build a 6-Month Emergency Fund', description: 'Before aggressive investing, secure 6 months of living expenses in a high-yield savings account. This is your shield against chaos.', category: 'Saving', content: `
**Your emergency fund is not an investment; it is insurance.** It protects your actual investments from being sold at the worst possible time during a crisis.
...
- **When to Use It:** This fund is ONLY for true emergencies: job loss, unexpected medical bills, or urgent home repairs. It is not for a vacation or a new gadget.`
    ...
    { id: 'f3', title: 'The Power of Compound Interest', description: 'Understand that consistent, early investment is more powerful than large, late investments. Make time your greatest financial ally.', category: 'Investing', content: `
...
- **How to Apply It:** Start investing now, even if it's a small amount, in low-cost index funds (like VTI or VOO). Be consistent and patient.`
    ...
    { id: 'f4', title: 'Pay Yourself First', description: 'Automate transfers to your investment and savings accounts the day you get paid. Your future self is your most important creditor.', category: 'Mindset', content: `
**"Paying yourself first" is a mindset shift from saving what's left over to spending what's left after saving.**
...
// Component to render content with basic Markdown styling
  ...
  return (
    <div className="space-y-4 text-gray-300 leading-relaxed">
      {content.trim().split('\n').map((line, index) => {
        ...
        if (line.startsWith('### ')) {
          return <h3 key={index} className="text-xl font-bold text-forge-red font-header uppercase tracking-wider mt-6 mb-2">{line.substring(4)}</h3>;
        }
        if (line.startsWith('- ')) {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
         {activeTab === 'streams' && hustleList.map((hustle) => (
            <Card key={hustle.id} onClick={() => setSelectedItem(hustle)} className="group hover:bg-zinc-850 flex flex-col">
               ...
               <Button onClick={(e) => { e.stopPropagation(); setSelectedItem(hustle); }} variant="secondary" className="w-full group-hover:border-forge-red group-hover:text-white">
         ...
         {activeTab === 'guides' && guidesList.map((guide) => (
            <Card key={guide.id} onClick={() => setSelectedItem(guide)} className="group hover:bg-zinc-850 flex flex-col">
              ...
              <Button onClick={(e) => { e.stopPropagation(); setSelectedItem(guide); }} variant="secondary" className="mt-6 w-full group-hover:border-forge-red group-hover:text-white">
